...
bool **seven_seg_driver::draw() {
	for (int d = 0; d < display_length; d++) {
		...
		for (int s = 7; s >= 0; s--) {
			...
			else digitalWrite(seg_pins[7 - s], set_bit == !seg_low);
		}
		if (is_shift_reg) {
...
void seven_seg_driver::write(char *input) {
	for (int c = 0, d = 0; d <= display_length; c++, d++) {
		draw_buffer[c] = 0;
		switch (input[c])
		{
		...
		if (input[c] == 46 || input[c] == 44) d--, draw_buffer[d] = draw_buffer[d] + 128;		//ascii Dot, Comma
	}
}
